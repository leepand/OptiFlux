...
def scan_model_names(env, page=1, per_page=10):
    ...
    for model_name in os.listdir(env_dir):
        ...
        for model_version in os.listdir(model_dir):
            ...
            timestamp = datetime.fromtimestamp(timestamp, tz=pytz.utc).astimezone(beijing_tz)
            timestamp = timestamp.strftime("%Y-%m-%d %H:%M:%S")

            # 获取目录大小
            size = sum(os.path.getsize(os.path.join(version_dir, f)) for f in os.listdir(version_dir) if os.path.isfile(os.path.join(version_dir, f)))
        ...
        # 获取最大版本
        max_version = max(versions, key=lambda x: x["model_version"])["model_version"] if versions else "N/A"

        # 获取 Recomserver 和 Rewardserver 的生产端口及运行状态（示例代码）
        recomserver_port, recomserver_status = get_server_info(env, model_name, "Recomserver")
        rewardserver_port, rewardserver_status = get_server_info(env, model_name, "Rewardserver")
...
def scan_model_versions(env, model_name):
    ...
    for model_version in os.listdir(model_dir):
        ...
        recomserver_port, rewardserver_port = get_server_ports(env, model_name, model_version)
...
def get_server_ports(env, model_name, model_version):
    ...
    """
    # 示例：从配置文件中读取端口
    config_path = os.path.join(ENV_DIRS.get(env), model_name, model_version, "config.json")
    if os.path.exists(config_path):
        try:
            with open(config_path, "r") as f:
                config = json.load(f)
                return config.get("recomserver_port", "N/A"), config.get("rewardserver_port", "N/A")
        except Exception as e:
            print(f"Error reading config file: {e}")
    return "N/A", "N/A"
...
def handle_deploy2():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": f"Code deployed to {env} environment."})
    except Exception as e:
        # 打印异常信息和堆栈跟踪
        print(f"Error during deployment: {str(e)}")
        traceback.print_exc()  # 打印堆栈跟踪
...
def handle_logs(env):
    ...
    try:
        ...
        if not os.path.exists(log_dir):
            return jsonify({"status": "error", "message": "Log directory not found."}), 404
        ...
        if os.path.exists(LOG_DIR):
            for file in os.listdir(LOG_DIR):
                if file.endswith(".log"):
                    ...
                    # 转换为北京时间
                    beijing_tz = pytz.timezone('Asia/Shanghai')
                    modified_time_beijing = modified_time_utc.replace(tzinfo=pytz.utc).astimezone(beijing_tz)
                    # 格式化时间为字符串
                    modified_time_str = modified_time_beijing.strftime('%Y-%m-%d %H:%M:%S')
                    # 获取文件大小（转换为MB或KB）
